; Bounded shutdown-drain counterexample query for one current Context.
;
; Bound: stop publication, listener/read revocation, zero or more summarized
; write-ready/service cycles for one finite Buffer, callback execution, handler
; finally, then retirement.  Repeated partial writes are summarized by the
; explicit finite-write-success assumption below.
; accept_admitted_after_stop/read_admitted_after_stop mean acquisition of those
; reactor actions after the stop CAS.  An action acquired before the CAS may
; finish its internal dispatches before the single reactor enters cleanup.
...
; Explicit bounded progress assumptions.  These are not scheduler-free facts.
...
; request-stop! and ordinary actions race through one owner-independent CAS
; state.  A pre-CAS action may remain active after the phase changes, but the
; reactor cannot enter cleanup until that already-running action returns and its
; finally decrements :active.  Release never reopens the stopping phase.
...
; cleanup-server! does not retire Context sockets until quiesce-handlers! has
; observed every WORKING bit clear.
...
; This equality is deliberately success-only.  The implementation does not run
; a Buffer completion callback when try-write throws.
